...
$(function(){
    ...
    $(grid_selector).jqGrid({
        url:"path/queryPathList",
        datatype: "json",
        mtype: 'POST',
        height:window.screen.height-550,
        colModel: [
            { label: 'id', name: 'id', width: 75,hidden:true},
            { label: '编号', name: 'pathId', width: 80 },
            { label: '长度（m）', name: 'distance', width: 80 },
            { label: '监测点个数', name: 'monitor', width: 80 },
            { label: '起点', name: 'begin', width: 80 },
            { label: '终点', name: 'end', width: 80 },
            { label: '示意图', name: 'photo', width: 80 ,hidden:true},
            { label: '示意图', name: 'opt', width: 80,formatter: function(cellvalue, options, cell){
    ...
    $("#queryBtn").click(function(){
        var qryPathId=$("#qryPathId").val();
        var qryBegin=$("#qryBegin").val();
        var qryEnd=$("#qryEnd").val();
        $(grid_selector).jqGrid('setGridParam',{
            postData:{path_id:qryPathId,begin:qryBegin,end:qryEnd},
            //search: true,
            page:1
        }).trigger("reloadGrid");
    });
